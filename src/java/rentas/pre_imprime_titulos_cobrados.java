 ...
 * @author byron
 */
public class pre_imprime_titulos_cobrados extends Pantalla {
    ...
    public pre_imprime_titulos_cobrados() {
        ...
        bot_anular.setValue("Cambio Estado");
        bot_anular.setMetodo("cambiar_estado_titulo");
        bar_botones.agregarBoton(bot_anular);
       

        tab_sql.setId("tab_sql");
        tab_sql.setSql("select a.ide_titulo,e.ide_estado,e.detalle,fecha_pago,des_ingreso,parroquia,calles,observaciones,(valor +(multa +interes -descuento) ) as valor_total,descuento,multa,interes "
                ...
                + "and a.ide_estado=e.ide_estado "
                + "order by des_ingreso");
        
        tab_sql.setCampoPrimaria("ide_titulo");
    ...
    public void cancelarDialogo() {
        if (dia_cambio_estado.isVisible()) {

...
public class Utilitario implements Serializable {
    ...
    public Conexion getConexion() {
        ...
        try {
            conexion = (Conexion) FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get("CONEXION");
    ...
    public boolean tieneCampoEmpresa(String tabla, String campoPrimaria) {
        //Retorna si una tabla tiene IDE_EMPR     
        String sql = "SELECT * FROM " + tabla + " WHERE " + campoPrimaria + "=-1";
        try {
            getConexion().setSentencia(getConexion().getConexion().createStatement());
            ResultSet resultado = getConexion().getSentencia().executeQuery(sql);
            ResultSetMetaData rsm_metadata = resultado.getMetaData();
            for (int i = 1; i <= rsm_metadata.getColumnCount(); i++) {
                if (rsm_metadata.getColumnName(i).equalsIgnoreCase("IDE_EMPR")) {
                    return true;
                }
            }
            resultado.close();
            getConexion().getSentencia().close();
        } catch (Exception e) {
        }
        return false;
    }
    ...
    public Tabla consultar(String sql) {
    ...
    public String getCampoEmpresa(String campo) {
        String valor = null;
        Tabla tab_empresa = consultar("SELECT IDE_EMPR," + campo + " FROM SIS_EMPRESA WHERE IDE_EMPR=" + getVariable("IDE_EMPR"));
    ...
    public void buscarNombresVisuales(Tabla tabla) {
        Tabla tab_campos = consultar("SELECT * FROM sis_campo WHERE ide_tabl in(SELECT ide_tabl FROM sis_tabla WHERE lower(tabla_tabl)='" + tabla.getTabla().toLowerCase() + "')");
